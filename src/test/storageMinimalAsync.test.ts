...
import {
    StorageMinimalAsync
} from '../storage/storageMinimalAsync';
...
t.test(`StorageMinimalAsync: constructor success`, (t: any) => {
    let storage = new StorageMinimalAsync(VALIDATORS, WORKSPACE);
    t.same(storage.workspace, WORKSPACE, 'it is working');
    storage.close();
    t.end();
});

t.test(`StorageMinimalAsync: constructor errors`, (t: any) => {
    t.throws(() => new StorageMinimalAsync([], WORKSPACE), 'throws when no validators are provided');
    t.throws(() => new StorageMinimalAsync(VALIDATORS, 'bad-workspace-address'), 'throws when workspace address is invalid');
    t.end();
});
...
let scenarios : Scenario[] = [
    {
        ...
        description: "StorageMinimalAsync",
    },
];
