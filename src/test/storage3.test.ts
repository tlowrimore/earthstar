...
for (let scenario of scenarios) {
    ...
    t.test(scenario.description + ': sync: mismatched workspaces', (t: any) => {
        ...
        t.same(storage3Sync(storageA1, storageB),  { numPulled: 0, numPushed: 0}, 'sync across different workspaces should do nothing');
        t.same(storage3Sync(storageA1, storageA2), { numPulled: 1, numPushed: 1}, 'sync across matching workspaces should do something');
    ...
    t.test(scenario.description + ': sync: misc other options', (t: any) => {
        ...
        t.same(storage3Sync( storageEmpty1, storageEmpty2), { numPushed: 0, numPulled: 0 }, 'sync with empty stores');
        t.same(storage3Push( storageEmpty1, storageEmpty2), 0, 'push with empty stores');
        t.same(storage3Push( storageEmpty1, storage      ), 0, 'push from empty to full store');

        // sync with self
        t.same(storage3Sync(storage, storage), { numPushed: 0, numPulled: 0 }, 'sync with self should do nothing');

        // successful sync
        t.same(storage3Sync(storage, storageEmpty1), { numPushed: 1, numPulled: 0 }, 'successful sync (push)');
        t.same(storage3Sync(storageEmpty2, storage), { numPushed: 0, numPulled: 1 }, 'successful sync (pull)');

        t.same(storage3Push(storage, storageEmpty3), 1, 'successful push');
    ...
    t.test(scenario.description + ': onWrite', (t: any) => {
        ...
        let numSynced = storage3Push(storage4, storage);
